package com.example.shmrapp.core.ui.viewmodels

import android.util.Log
import androidx.lifecycle.ViewModel
import androidx.lifecycle.viewModelScope
import com.example.shmrapp.core.ext.toUiMessage
import com.example.shmrapp.core.ui.models.TransactionForHistoryModel
...
data class ExpenseHistoryState(
    ...
    val amount: Int = 0,
    val transactions: List<TransactionForHistoryModel> = emptyList() // Добавим список транзакций
)
...
class TransactionHistoryViewModel(
    ...
    private fun loadTransactions() {
        viewModelScope.launch {
            ...
            var countedAmount = 0
            getTransactionsByPeriodUseCase.execute(
                accountId = 211, // Фиксированный accountId
                startDate = _state.value.startDate,
                endDate = _state.value.endDate,
                isIncome = isIncome, // Фильтр только расходов
            )
                .onSuccess { successTransactions ->
                    val mappedTransactions = successTransactions.filter { it.category.isIncome == isIncome }.map {
                        countedAmount += it.amount.toDouble().toInt()
                        TransactionForHistoryModel(
